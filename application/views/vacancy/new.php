								...
								<div class="col-lg-12">
									...
									<div class="mb-5">
										...
										<div class="form-check form-check-solid mb-5">
										    <input class="form-check-input" type="checkbox" value="<?=$row->id?>" name="tests[]" onchange="selecttest('<?=$row->id?>',<?=$row->test_credit?>,this)"/>
										    <label class="form-check-label">
										        <?=$row->name." (".$row->test_credit." Credit)"?>
										    </label>
										</div>
										<?php if($row->evaluation!=null){ ?>
										<div class="row d-none" id="test-target-<?=$row->id?>">
											<?php 
											...
											foreach($targets as $key=>$optstr){
												...
												<div class="col-lg-6 form-check-inline">
													...
													<?php $i=0;foreach($opts as $opt){?>
														<input class="form-check-input d-inline" type="radio" value="<?=$opt?>" name="testtargets[<?=$row->id?>][<?=$key?>]" <?php if($i===0)echo "checked"?>/>
